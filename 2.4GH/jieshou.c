...
//delay_ms(10);
	...
	SPI_Write_Reg(WRITE_REG+NRF_CONFIG,0x0e);//IRQ收发完成中断响应，16位CEC,发送模式
	CE=1;//置高CE，激发数据发送
...
SPI_Write_Reg(WRITE_REG+NRF_CONFIG,0x0f);//IRQ中断响应，16位CRC校验，接收模式
CE=1;//上电，接收
delay_ms(200);//接收延时，这个时间要控制得恰到好处
//delay_ms(900);
}

//++++++++++++++++++++++++++
//数据读取后放入到rx_buf数组接收缓冲区中
//++++++++++++++++++++++++++
uchar NRF24L01_RxPacket(uchar * rx_buf)
{uchar revale=0;
uchar sta;
sta=SPI_Read_Reg(STATUS);//读取状态寄存器来判断数据接收状况
if(RX_DR)
	{
	CE=0;//SPI使能
	SPI_Read_Buf(RD_RX_PLOAD,rx_buf,RX_PLOAD_WIDTH);//读取接收的数据
	revale=1;//读取数据完成标志
	SPI_Write_Reg(WRITE_REG+STATUS,0xff);//写1来清除中断标志
	}
return revale;
}
...
{	
	uchar i=0;
	uchar Buf0[5]={0,0,0,0,0};//这个用来装数据，
	uchar status;//用来装读取到的status的数值
    uchar fifo_s;//定义一个变量用来存储读取到的FIFO的状态
	init_NRF24L01();//初始化
	SPI_Read_Buf(RX_ADDR_P0,Buf0,RX_ADR_WIDTH);//读取通道0的数据放置在Buf0中
	delay_ms(100);
		
	status=0;//status置0,用于保存后面读取的数据
//	delay_ms(100);
	shownews("ceshi2","ceshi2");//用以显示测试
	delay_ms(500);//延时测试LED
	shownews(uchartostr(Buf0[0]),uchartostr(Buf0[0]));//显示通道0还没有输入数据前的状态
	delay_ms(700);
	shownews(uchartostr(Buf0[1]),uchartostr(Buf0[1]));
	delay_ms(700);
...
//	status=SPI_Read_Reg(0x07);//读取寄存器0x07的值
	...
	shownews(uchartostr(status),uchartostr(status));//分别显示STATUS和FIFO_STATUS
...
//	NRF24L01_TxPacket(TxBuf);//进行一次数据（TxBuf）的发送
	...
	shownews("123456",uchartostr(status));//分别显示STATUS和FIFO_STATUS
	delay_ms(1000);//延时，用这段时间来记录
//	shownews(uchartostr(status),"123456");//显示status的值
	delay_ms(1000);
//	shownews("123456",status);
*/
	setRx_Mode();//进行一次接收，这个频率一定要快
//	shownews("js","ing");//显示js 和ing
	i=SPI_Read_Reg(CD);//读取模块的载波检测信息
//	shownews("CD","CD");
if(i==1)
{
shownews("CD",uchartostr(i));//显示检测到的载波信息,1就是收到载波
delay_ms(1000);
delay_ms(1000);
}
else
{
shownews("CD",uchartostr(i));//显示检测到的载波信息,0就是没收到载波
delay_ms(1);
}
